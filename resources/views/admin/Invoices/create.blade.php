...
@section('customejs')
    <script>
        ...
        $("#productselection").on('select2:select', function (selection) {
            if ($("#r_" + selection.params.data.id).length) {
                quant = $("#r_" + selection.params.data.id).find('#td_quantity');
                price = parseFloat($("#r_" + selection.params.data.id).find('#td_price').text());
                quant.val(parseInt(quant.val()) + 1);
                var  total = $("#r_" + selection.params.data.id).find('#td_total');
                total.text(eval(parseInt(quant.val()) * price));
                $('#productselection').val(null).trigger('change');
                $("#productselection").val("").trigger("change");
                $("#productselection").trigger("change");
                calcluaterows();
                return ;
            }
            ...
            $("#product_Body").append(
                "<tr class='text-center' role='row' class='odd' id='r_" + selection.params.data.id + "'>" +
                "<td  id='td_name'>" + selection.params.data.name + "</td>" +
                "<td  id='td_barcode'>" + selection.params.data.barcode + "</td>" +
                "<td >" + "<input type='number' class='text-center' id='td_quantity' value='1' min='0' step='1'>" + "</td>" +
                "<td  id='td_price'>" + selection.params.data.sail_price + "</td>" +
                "<td  id='td_total'>" + selection.params.data.sail_price + "</td>" +
                "<td >" + "<span class='text-center'><a  id='td_action' href='#' class='text-red'><i class='fas fa-plus fa-trash' aria-hidden='true'></i></a></span>" + "</td>" +
                "</tr>"
            );
            calcluaterows();
