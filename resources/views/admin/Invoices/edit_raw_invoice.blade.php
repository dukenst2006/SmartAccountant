            ...
            <div class="col-10">
                <div class="card" style="font-family: 'Cairo', sans-serif;font-weight: 900;">
                    <div class="card-header">
                        <h3 class="card-title">تعديل فاتورة كود: {{ $invoice->invoice_code }}</h3>
                    </div>
                    
                    <div class="card-body p-0 ">
                        {!! Form::open(['route' => ['admin.raw_invoice.update', $invoice->id], 'method' => 'put']) !!}
                            
                            {!! Form::token() !!} 

                            <div class="form-group row col-sm-6 m-3">
                                {!! Form::label('MarketplaceID', __('Models/Marketplace.Name')) !!}
                                {!! Form::select('MarketplaceID',$marketplaces, $invoice->MarketplaceID,['class' => 'form-control']) !!}
                            </div>

                            <div class="form-group row col-sm-6 m-3">
                                {!! Form::label('PaymentTypeID', __('Models/Invoice.PaymentTypeID')) !!}
                                {!! Form::select('PaymentTypeID',$payment_types, $invoice->PaymentTypeID,['class' => 'form-control']) !!}
                            </div>

                            <div class="form-group row col-sm-6 m-3">
                                {!! Form::label('CustomerName', __('Models/Invoice.Name')) !!}
                                {!! Form::text('CustomerName', $invoice->CustomerName, ['class' => 'form-control']) !!}
                            </div>

                            <div class="form-group row col-sm-6 m-3">
                                {!! Form::label('Total', __('Models/Invoice.Total')) !!}
                                {!! Form::text('Total', $invoice->Total, ['class' => 'form-control']) !!}
                            </div>
