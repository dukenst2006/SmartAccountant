<!-- Marketplaceownerid Field -->
<div class="form-group">
    {!! Form::label('MarketplaceOwnerID', 'Marketplaceownerid:') !!}
    <p>{{ $marketplace->MarketplaceOwnerID }}</p>
</div>

<!-- Name Field -->
<div class="form-group">
    {!! Form::label('Name', 'Name:') !!}
    <p>{{ $marketplace->Name }}</p>
</div>

<!-- Country Field -->
<div class="form-group">
    {!! Form::label('Country', 'Country:') !!}
    <p>{{ $marketplace->Country }}</p>
</div>

<!-- City Field -->
<div class="form-group">
    {!! Form::label('City', 'City:') !!}
    <p>{{ $marketplace->City }}</p>
</div>

<!-- Supervisorphonenumber Field -->
<div class="form-group">
    {!! Form::label('SupervisorPhoneNumber', 'Supervisorphonenumber:') !!}
    <p>{{ $marketplace->SupervisorPhoneNumber }}</p>
</div>

<!-- Address Field -->
<div class="form-group">
    {!! Form::label('Address', 'Address:') !!}
    <p>{{ $marketplace->Address }}</p>
</div>

<!-- Taxnumber Field -->
<div class="form-group">
    {!! Form::label('TaxNumber', 'Taxnumber:') !!}
    <p>{{ $marketplace->TaxNumber }}</p>
</div>

<!-- Email Field -->
<div class="form-group">
    {!! Form::label('Email', 'Email:') !!}
    <p>{{ $marketplace->Email }}</p>
</div>

<!-- Latitude Field -->
<div class="form-group">
    {!! Form::label('Latitude', 'Latitude:') !!}
    <p>{{ $marketplace->Latitude }}</p>
</div>

<!-- Longitude Field -->
<div class="form-group">
    {!! Form::label('Longitude', 'Longitude:') !!}
    <p>{{ $marketplace->Longitude }}</p>
</div>

<!-- Safebalance Field -->
<div class="form-group">
    {!! Form::label('SafeBalance', 'Safebalance:') !!}
    <p>{{ $marketplace->SafeBalance }}</p>
</div>

<!-- Companyregisterimage Field -->
<div class="form-group">
    {!! Form::label('CompanyRegisterImage', 'Companyregisterimage:') !!}
    <p>{{ $marketplace->CompanyRegisterImage }}</p>
</div>

<!-- Logo Field -->
<div class="form-group">
    {!! Form::label('Logo', 'Logo:') !!}
    <p>{{ $marketplace->Logo }}</p>
</div>

